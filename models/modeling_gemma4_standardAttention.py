...
class Gemma4TextAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        attn_output = query_states.new_empty((bsz, q_len, n_heads, self.head_dim))  # interface returns [B, q, H, d]
        block_size = 256
        for s in range(0, q_len, block_size):
            e = min(s + block_size, q_len)
            u = kv_len - q_len + e
            mask_tile = attention_mask[:, :, s:e, :u] if attention_mask is not None else None  # slice dim 2
            out_tile, _ = attention_interface(
                self,
                query_states[:, :, s:e],  # tile queries only
                key_states[:,:,:u], value_states[:,:,:u],
                mask_tile,
                dropout=self.attention_dropout if self.training else 0.0,
                scaling=self.scaling,
